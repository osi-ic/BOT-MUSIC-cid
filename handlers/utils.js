...
module.exports = async (client) => {
  ...
  client.playembed = (guild) => {
    let embed = new MessageEmbed()
      .setColor(client.config.embed.color)
      .setTitle(`Ingin dengerin musik tinggal ketik aja nama lagunya di bawah atau tinggal paste aja link url musik yang mau kalian dengerin`)
      
      .setImage(
        guild.banner
          ? guild.bannerURL({ size: 4096 })
          : `https://media.discordapp.net/attachments/983291751060086798/983402358585299074/simon-noh-0rmby-3OTeI-unsplash.jpg`
      )
      .setFooter({
        text: guild.name,
        iconURL: guild.iconURL({ dynamic: true }),
